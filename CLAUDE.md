...
|---|---|---|---|---|---|---|---|
| 2026-09-23 14:21 | 真机 | 1m34s | 6,430,407 | 98.1% | 30.3% | +2,748 B | 3782dbfa |
...
### 新会话启动标准（2026-09-23 固化）

**开新会话时，AI 必须：**

1. 用户贴出「任务启动文档」全文（或指向 docs/<task>.md）
2. AI 先读两份文件：
   - docs/<task>.md（任务自包含文档）
   - CLAUDE.md（本文件 · 完整开发规范）
3. 按任务文档的「待探测」段先跑探测命令，**禁止跳过探测直接写代码**
4. 按任务文档的「实现方案」分阶段执行
5. 遵守本文件所有强制约定（模拟器优先 / 备份 / printf 写文档 / 五步提交）
6. 遵守安全红线（eFuse / Secure Boot / Flash 加密 / 生产模式 禁写）

**任务启动文档标准模板**（放在 docs/<task>.md）：
...
- 四、实现方案（分阶段 + 关键技术点）
- 五、强制约定（指向 CLAUDE.md）
- 六、编译状态（当前 Flash/RAM/commit）
- 七、用户偏好
- 八、工作流规范（提交 / 文档写入 / 编译记录 / 烧录）
- 九、安全红线
- 十、新会话第一件事
